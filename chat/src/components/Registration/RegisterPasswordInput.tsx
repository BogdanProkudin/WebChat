...
const RegisterPasswordInput: React.FC<IRegisterInput> = ({ register, errors }) => {
  const [showPassowrd, setShowPassword] = useState<boolean>(false);
  const Passwordregex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).+$/;
  return (
    <div>
      <img className="Registerlock-Image" src={lock} alt="lock" />
      <input
        {...register('password', {
      ...
      {errors?.password?.message && (
        <span className="wrongPassword">{errors.password.message.toString() || 'error'}</span>
      )}
